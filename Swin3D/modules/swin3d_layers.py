...
def sparse_self_attention(w_w_id: torch.Tensor, w_sizes: torch.Tensor, protocol: str = 'v1'):
    ...
    w2n_indices = torch.cat([torch.zeros(1, dtype=w_cumsum.dtype, device=w_cumsum.device), w_cumsum[:-1]])

    # w2m indices - [W], mapping window index to window global offset in output
    #   space
    w2_cumsum = torch.cumsum(w_sizes_2, dim=-1)
    w2m_indices = torch.cat([torch.zeros(1, dtype=w2_cumsum.dtype, device=w2_cumsum.device), w2_cumsum[:-1]])

    # m2w indices - [M], mapping element global offset to the window index
    m2w_indices = torch.zeros([w2_cumsum[-1]], dtype=w_sizes.dtype, device=w_sizes.device)
    m2w_offset = torch.zeros([w2_cumsum[-1]], dtype=w_sizes.dtype, device=w_sizes.device)
    ...
    m_indices = torch.arange(0, w2_cumsum[-1], dtype=w_sizes.dtype, device=w_sizes.device)
...
class GridCoordsDown(nn.Module):
    ...
    def __init__(self, stride):
        super().__init__()
        self.stride = stride
        self.avg_pool = ME.MinkowskiAvgPooling(kernel_size=self.stride, stride=self.stride, dimension=3)
        self.unpool = ME.MinkowskiPoolingTranspose(kernel_size=stride, stride=stride, dimension=3)
        self.max_pool = ME.MinkowskiMaxPooling(kernel_size=self.stride, stride=self.stride, dimension=3)
    ...
    def forward(self, coords_sp, sp, return_map=False):
        ...
        map_pair = sp.coordinate_manager.kernel_map(dist_sp.coordinate_map_key, min_dist_sp.coordinate_map_key, stride=self.stride, kernel_size=self.stride, is_pool=True)[0]
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, quant_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., cRSE='XYZ_RGB', fp16_mode=0):
        ...
        if 'XYZ' in cRSE:
            ...
            trunc_normal_(self.query_xyz_table, std=.02)
            self.key_xyz_table = nn.Parameter(torch.zeros(table_shape_xyz))
            trunc_normal_(self.key_xyz_table, std=.02)
            self.value_xyz_table = nn.Parameter(torch.zeros(table_shape_xyz))
            trunc_normal_(self.value_xyz_table, std=.02)
        ...
        if 'RGB' in cRSE:
            ...
            trunc_normal_(self.query_rgb_table, std=.02)
            self.key_rgb_table = nn.Parameter(torch.zeros(table_shape_rgb))
            trunc_normal_(self.key_rgb_table, std=.02)
            self.value_rgb_table = nn.Parameter(torch.zeros(table_shape_rgb))
            trunc_normal_(self.value_rgb_table, std=.02)
        ...
        if 'NORM' in cRSE:
            ...
            trunc_normal_(self.query_norm_table, std=.02)
            self.key_norm_table = nn.Parameter(torch.zeros(table_shape_norm))
            trunc_normal_(self.key_norm_table, std=.02)
            self.value_norm_table = nn.Parameter(torch.zeros(table_shape_norm))
            trunc_normal_(self.value_norm_table, std=.02)
    ...
    def forward(self, feats: torch.Tensor, attn_args):
        ...
        x_offset, y_offset, m2w_indices, w_sizes, w2n_indices, n2n_indices, w2m_indices,\
            n_coords = attn_args
      
        # Query, Key, Value
        qkv = self.qkv(feats)
        qkv = qkv.reshape(num_v, 3, self.num_heads, num_sc).permute(1, 0, 2, 3).contiguous()
        ...
        updated_values = SelfAttnAIOFunction.apply(query, key, value, query_table, key_table, value_table, table_offsets, indices, \
            PosEmb.SEPARATE, TableDims.D0, IndexMode.INDIRECT, fp16_mode)
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size, quant_size, drop_path=0.0,\
            mlp_ratio=4.0, qkv_bias=True, qk_scale=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm, cRSE='XYZ_RGB', fp16_mode=0):
        super().__init__()
        self.window_size = window_size

        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention(dim, window_size=self.window_size, quant_size=quant_size, num_heads=num_heads, \
            qkv_bias=qkv_bias, qk_scale=qk_scale, cRSE=cRSE, fp16_mode=fp16_mode)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, window_size, quant_size, out_channels=None,
            mlp_ratio=4., qkv_bias=True, qk_scale=None,
            drop_path=0., norm_layer=nn.LayerNorm, downsample=None, down_stride=2, cRSE='XYZ_RGB', fp16_mode=0):
        super().__init__()
        self.window_size = window_size
        self.depth = depth
        self.dim = dim
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                dim, num_heads, window_size, quant_size,
                drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,\
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, norm_layer=norm_layer, cRSE=cRSE, fp16_mode=fp16_mode)
            for i in range(depth)])

        self.pool = ME.MinkowskiMaxPooling(kernel_size=self.window_size, stride=self.window_size,dimension=3)
    ...
    def get_map_pair(self, sp):
        ...
        x, y, z = [torch.arange(window_size[i], device=self.device)*stride_in[i] for i in range(3)]
        ...
        query_key, (map, inverse_map) = cm.insert_and_map(all_windows, tensor_stride=stride_in)
    ...
    def get_window_mapping(self, sp):
        ...
        inv_sort_idx[sort_idx] = torch.arange(sort_idx.shape[0], dtype=sort_idx.dtype, device=self.device)
        N = window_N * nW
        v2w_mask = torch.zeros(N, dtype=torch.bool, device=self.device)
        w_id = torch.arange(window_N, dtype=torch.long, device=self.device).unsqueeze(1).repeat(1, nW).view(-1)
        w_w_id = torch.arange(nW, dtype=torch.long, device=self.device).unsqueeze(0).repeat(window_N, 1).view(-1)
        v2w_mask[in_map.long()] = True
        nempty_num = v2w_mask.view(-1, nW).sum(dim=-1)
        w_id = w_id[in_map.long()]
        w_w_id = w_w_id[in_map.long()]
        w_w_xyz = torch.stack([w_w_id//window_size//window_size, w_w_id//window_size%window_size, w_w_id%window_size], dim=-1)
    ...
    def get_index01(self, sp, local_xyz, colors):
        ...
        return x_offset, y_offset, m2w_indices, w_sizes, w2n_indices, n2n_indices,\
    ...
    def get_shifted_sp(self, sp):
        ...
        shifted_sp = SparseTensor(features=sp.F, coordinates=shifted_C, device=self.device, tensor_stride=stride_in)
        return shifted_sp
